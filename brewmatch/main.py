...
def load_data():
    global users, requests, requests_human, connections, bot_chats, messages
...
async def request_chat(req: Dict):
    ...
    request_type = req.get("request_type", "bot") # 'bot' or 'human'
    
    if not sender_id or not recipient_id:
        raise HTTPException(status_code=400, detail="Missing IDs")
    
    target_requests = requests_human if request_type == 'human' else requests
    
    if recipient_id not in target_requests:
        target_requests[recipient_id] = []
...
async def save_report(req: Dict):
    ...
    # Store with bidirectional key sorting
...
async def respond_request(req: Dict):
    ...
    if user_id in requests and sender_id in requests[user_id]:
        ...
        if action == 'accept':
            # Add to both bot_chats (Bot Match)
            if user_id not in bot_chats: bot_chats[user_id] = []
            if sender_id not in bot_chats: bot_chats[sender_id] = []
            if sender_id not in bot_chats[user_id]: bot_chats[user_id].append(sender_id)
            if user_id not in bot_chats[sender_id]: bot_chats[sender_id].append(user_id)
        save_data()
        return {"success": True}
